...
graph <- function(x, y, data, geom, title){
  aes <- eval(substitute(aes(x, y),
    list(x = substitute(x), y = substitute(y))))
  p <- ggplot(data, aes) + geom + ggtitle(title)
}

q <-graph(x = contb_receipt_amt, data = p,
      geom = geom_histogram(binwidth = 10,col = "black", fill = "#6495ED"),
      title = "number of donations at each amount interval")
q+scale_x_continuous(lim = c(0,1000),breaks = seq(0,1000,50))
```
...
q <-graph(x = contb_receipt_amt, data = p,
      geom = geom_histogram(col = "black", fill = "#6495ED",binwidth = 0.05),
      title = "number of donations at each mount log10 interval")
q+scale_x_log10(breaks = c(5,10,50,100,200,500,1000,5000,10000))
