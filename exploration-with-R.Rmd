...
del_name_list = c("cmte_id","cand_id", "contbr_st","file_num","tran_id","form_tp")
...
p$contb_receipt_dt <- as.Date(p$contb_receipt_dt, format = "%d-%b-%y")     #switch date from factor to Date format
p$contbr_zip <- substr(p$contbr_zip,1,5)      #extract the first 5 digit number as zip codes.
...
```{r,echo = FALSE}
#create lists to group them
Republican <- c("Bachmann, Michele","Cain, Herman","Gingrich, Newt" ,"Huntsman, Jon","McCotter, Thaddeus G","Paul, Ron","Pawlenty, Timothy","Perry, Rick","Roemer, Charles E. 'Buddy' III","Romney, Mitt","Santorum, Rick")
Libertarian <- c("Johnson, Gary Earl")
Democratic <- c("Obama, Barack")
Green <- c("Stein, Jill")
#create the new variable
p$cand_party <- ifelse(p$cand_nm%in%Republican,"Republican", ifelse(p$cand_nm%in%Libertarian,"Libertarian", ifelse(p$cand_nm%in%Democratic,"Democratic", "Green")))
...
```{r, warining = FALSE,echo = FALSE}
ggplot(aes(x = contb_receipt_amt),data = subset(p,p$contb_receipt_amt > 0))+
...
ggplot(aes(x = cand_party,fill=ele_tp),
       data = p_tem[p_tem$cand_party == "Democratic" | p_tem$cand_party == "Republican",])+
  geom_bar(stat = "count")+
  ggtitle("Donation number vs party by election type")

#group data by election type and party, then sum the amount of money.
p_by_eletp <- subset(p[p$cand_party == "Democratic" | p$cand_party == "Republican",]) %>%
...
p_rich <- subset(p_rich,p_rich$cand_party == "Democratic" | p_rich$cand_party == "Republican")

ggplot(aes(x = contb_receipt_dt, y = contb_receipt_amt, col = ele_tp), data = p_rich)+
...
# group dataset by city and party, and then reshape it. The value I care about is the count number
p_by_city <- p %>%
...
# group dataset by city and party, and then reshape it. The value I care about is the donation amount
p_by_city_amt <- p %>%
  group_by(contbr_city,cand_party) %>%
  summarize(sum_amt = sum(contb_receipt_amt)) %>%
  arrange(desc(sum_amt))
...
p_ocp <- p[(p$contbr_occupation != "" & p$contbr_occupation != "INFORMATION REQUESTED PER BEST EFFORTS"),]
p_ocp$contbr_retire <- "employed"
p_ocp$contbr_retire[grep("^RETIRED.*",p_ocp$contbr_occupation)] <- "retired"
p_ocp$contbr_retire[grep("^NOT EMPLOYED.*", p_ocp$contbr_occupation)] <- "unemployed"
...
ggplot(aes(x = contbr_retire, fill = cand_party), 
       data = p_ocp[p_ocp$cand_party == "Democratic" | p_ocp$cand_party == "Republican",])+
...
p_occ$contbr_occupation <- replace(p_occ$contbr_occupation, 
                                   p_occ$contbr_occupation == "LAWYER","ATTORNEY")
p_occ$contbr_occupation <- replace(p_occ$contbr_occupation, 
                                   p_occ$contbr_occupation%in%c("PROFESSOR","COLLEGE PROFESSOR"),"TEACHER")
...
p_occ_wide_n <- dcast(p_occ_new,
                    ...
                    value.var = "n")
ggplot(aes(x = Democratic, y = Republican,label = contbr_occupation), data = p_occ_wide_n)+
  geom_point()+
  geom_text_repel()+
  geom_abline(intercept = 0,slope = 1)+
...
```{r,echo = FALSE}
ggplot(aes(x = cand_nm, y = sum_amt/n,fill = cand_party, angle = 30), data = p_by_candnm)+
  geom_bar(stat = "identity")+
  ggtitle("Average donation per donator for each candidate")+
